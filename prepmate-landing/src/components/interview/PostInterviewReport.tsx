...
import type { Message, FeedbackMetrics } from "../../types/interview";
...
const PostInterviewReport: React.FC<PostInterviewReportProps> = ({
  ...
  const totalFillerWords = Array.from(metrics.fillerWords.values()).reduce(
    (sum: number, count: number) => sum + count,
    0
  );
  const averageSentiment =
    metrics.sentimentHistory.length > 0
      ? metrics.sentimentHistory.reduce((sum, s) => sum + s.score, 0) /
        metrics.sentimentHistory.length
      : 0;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-80 flex justify-center items-center z-50 p-4 transition-opacity duration-300">
      <div
        className={`bg-gray-800 rounded-lg p-8 max-w-4xl w-full h-full max-h-[90vh] flex flex-col shadow-2xl transform transition-all duration-500 ease-out ${
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div className="bg-gray-700 p-4 rounded-lg text-center">
              ...
              <p className="text-2xl font-bold text-white">
                {metrics.pace > 0 ? metrics.pace.toFixed(0) : "N/A"} WPM
              </p>
            </div>
            <div className="bg-gray-700 p-4 rounded-lg text-center">
          ...
          <div className="bg-gray-900 bg-opacity-50 p-6 rounded-lg">
            ...
            <div className="space-y-4 max-h-72 overflow-y-auto pr-2">
              {conversation.map((msg, index) => (
                ...
                >
                  {msg.author === "ai" && (
                    <div className="w-8 h-8 rounded-full bg-cyan-800 flex-shrink-0 flex items-center justify-center font-bold text-sm">
                      AI
                    </div>
                  )}
                  <div
                    className={`p-3 rounded-lg max-w-lg ${
                      msg.author === "user" ? "bg-cyan-700" : "bg-gray-600"
                    }`}
                  >
                    <p>{msg.text}</p>
                  </div>
